...
exports.PRECISION = exports.SECONDS_PER_YEAR = void 0;
exports.calculateExchangeRate = calculateExchangeRate;
exports.calculateIntervalRateFromApy = calculateIntervalRateFromApy;
exports.calculateAnnualYieldBps = calculateAnnualYieldBps;
exports.verifyIntervalRate = verifyIntervalRate;
exports.calculateLstExchangeRate = calculateLstExchangeRate;
exports.calculateLstAmount = calculateLstAmount;
exports.calculateBaseAmount = calculateBaseAmount;
exports.verifyWithdrawalAmounts = verifyWithdrawalAmounts;
exports.testWithdrawalAmounts = testWithdrawalAmounts;
exports.extractAprFromIntervalRate = extractAprFromIntervalRate;
exports.calculateExchangeRateWithoutCompounding = calculateExchangeRateWithoutCompounding;
const date_fns_1 = require("date-fns");
// Constants
exports.SECONDS_PER_YEAR = 365 * 24 * 60 * 60;
exports.PRECISION = 1000000000000n;
/**
 ...
 */
function calculateExchangeRate(lastYieldChangeTimestamp, currentTimestamp, intervalRate, lastYieldChangeExchangeRate, intervalSeconds) {
    ...
    let compoundedRate = exports.PRECISION;
    for (let i = 0n; i < intervalAmounts; i++) {
        compoundedRate = (compoundedRate * interval) / exports.PRECISION;
    }
    // Calculate the linear yield for the remaining seconds
    const yieldPortion = interval - exports.PRECISION;
    ...
    const newExchangeRate = (lastExchangeRate * totalRate) / exports.PRECISION;
 ...
 */
function calculateIntervalRateFromApy(targetApyBps, intervalSeconds) {
 ...
 */
function calculateAnnualYieldBps(intervalRateParam, intervalSeconds) {
    // Constants
    const BPS_MULTIPLIER = 10000n;
    const SECONDS_PER_YEAR_BIG = 365n * 24n * 60n * 60n;
    // Higher internal precision for calculations (10x normal precision)
    const INTERNAL_PRECISION = exports.PRECISION * 10n;
    // Convert inputs to BigInt
    ...
    const yieldPerInterval = intervalRate > exports.PRECISION
        ? (intervalRate - exports.PRECISION) * 10n // Scale up for higher precision
        : 0n;
 ...
 */
function verifyIntervalRate(intervalRate, targetApyBps, intervalSeconds) {
 ...
 */
function calculateLstExchangeRate(pair, currentTimestamp) {
    return calculateExchangeRate(pair.lastYieldChangeTimestamp, currentTimestamp, pair.intervalAprRate, pair.lastYieldChangeExchangeRate, pair.secondsPerInterval);
}
/**
 ...
 */
function calculateLstAmount(baseAmount, exchangeRate, baseDecimals = 6, lstDecimals = 6) {
    ...
    const baseConversion = (amount * exports.PRECISION) / rate;
 ...
 */
function calculateBaseAmount(lstAmount, exchangeRate, baseDecimals = 6, lstDecimals = 6) {
    ...
    const lstConversion = (amount * rate) / exports.PRECISION;
    // Then adjust for decimal places difference
 ...
 */
function verifyWithdrawalAmounts(pair, lstAmount, currentTimestamp) {
    // Ensure timestamp is in seconds from Unix epoch
    const timestamp = currentTimestamp instanceof Date
        ? BigInt((0, date_fns_1.getUnixTime)(currentTimestamp))
    ...
    const baseAmount = calculateBaseAmount(lstBurnAmount, exchangeRate, Number(pair.baseMintDecimals), Number(pair.lstMintDecimals));
    return {
 ...
 */
function testWithdrawalAmounts(pair, requestedAmount, actualLstBurned, actualBaseAmount, currentTimestamp) {
 ...
 */
function extractAprFromIntervalRate(intervalRate, intervalSeconds) {
    // Constants
    const BPS_MULTIPLIER = 10000n;
    const SECONDS_PER_YEAR_BIG = 365n * 24n * 60n * 60n;
    // Convert inputs to BigInt
    ...
    const yieldPortion = rate > exports.PRECISION
        ? (rate - exports.PRECISION)
        : 0n;
    // Calculate APR in basis points
    // APR = yieldPortion * intervalsPerYear * BPS_MULTIPLIER / PRECISION
    const aprBps = (yieldPortion * intervalsPerYear * BPS_MULTIPLIER) / exports.PRECISION;
    return aprBps;
}
/**
 ...
 * @returns The calculated exchange rate as a bigint
 */
function calculateExchangeRateWithoutCompounding(lastYieldChangeTimestamp, currentTimestamp, intervalRate, lastYieldChangeExchangeRate, intervalSeconds) {
    // Convert all inputs to BigInt
    const lastTimestamp = BigInt(lastYieldChangeTimestamp);
    const currentTime = BigInt(currentTimestamp);
    const interval = BigInt(intervalRate);
    ...
    const totalRate = exports.PRECISION + linearYield;
    // Calculate the new exchange rate
    const newExchangeRate = (lastExchangeRate * totalRate) / exports.PRECISION;
    return newExchangeRate;
}
//# sourceMappingURL=rateCalculations.js.map